    ...
    def post(self, *args, **kwargs):
        ...
        if re.match(TEL_PATTERN, username):
            if not self.smscode_expired_or_not_exist(username):
                return self.common_response(FAILURE_CODE, "短信验证码未过期!")

            smscode = self.get_smscode()
            self.redis.set(username + SMSCODE_SUFFIX, smscode, ex=60 * 5)
            self.common_response(SUCCESS_CODE, "验证码获取成功", dict(smscode=smscode))

        else:
            self.common_response(FAILURE_CODE, "用户名非法!")
...
class RegisterHandler(BaseRequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        if AccountHelper.has_registered(username):
            return self.common_response(FAILURE_CODE, "该手机号码已经注册!")

        if not inner_smscode or inner_smscode != smscode:
            return self.common_response(FAILURE_CODE, "短信验证码输入错误或已经过期!")

        if not re.match(PASSWORD_PATTERN, password):
            return self.common_response(FAILURE_CODE, "非法的密码模式!")
        ...
        self.common_response(SUCCESS_CODE, "注册成功!", dict(token=token))
...
class LoginHandler(BaseRequestHandler):
    def post(self, *args, **kwargs):
        ...
        if not user:
            return self.common_response(FAILURE_CODE, "该用户不存在!")

        if not user.password == password:
            return self.common_response(FAILURE_CODE, "密码错误!")

        token = AccountHelper.generate_token(username)

        self.common_response(SUCCESS_CODE, "登录成功!", dict(token=token))
...
class ForgetHandler(BaseRequestHandler):
    def post(self, *args, **kwargs):
        ...
        if password == user.password:
            return self.common_response(FAILURE_CODE, "重置密码不可与原密码相同!")

        user.password = password
        user.save()

        self.common_response(SUCCESS_CODE, "密码重置成功!")
...
class PasswordHandler(BaseRequestHandler):
    def post(self, *args, **kwargs):
        ...
        if old_password == new_password:
            return self.common_response(FAILURE_CODE, "新旧密码不可相同!")

        if not re.match(PASSWORD_PATTERN, new_password):
            return self.common_response(FAILURE_CODE, "新密码模式非法!")
        ...
        self.common_response(SUCCESS_CODE, "密码修改成功!")
...
class InfoHandler(BaseRequestHandler):
    def post(self, *args, **kwargs):
        ...
        if not re.match(NICK_NAME_PATTERN, nick_name):
            return self.common_response(FAILURE_CODE, "昵称为4~18位字符!")

        if not isinstance(sex, int) or sex > 1 or sex < 0:
            return self.common_response(FAILURE_CODE, "性别不合法!")

        if age > 150 or age < 0:
            return self.common_response(FAILURE_CODE, "年龄不合法!")

        if province_id > 34 or province_id < 0:
            return self.common_response(FAILURE_CODE, "地区不合法!")
        ...
        self.common_response(SUCCESS_CODE, "信息修改成功!")
